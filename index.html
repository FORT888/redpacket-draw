...
<!-- Firebase CDN（compat 版便于直接 script 使用） -->
...
/* ---------- 配置（全部 USDT） ---------- */
const RATE = 7.2; // 1 USDT = 7.2 CNY（仅用于把人民币总额换成 USDT）
...
async function syncSave(){
  await db.collection("lottery").doc("roundState")
    .set({roundIndex,pool,history,userDraws,roundLocked},{merge:true});
}
...
/* ---------- 抽奖（多人可抽，直到本轮 pool 为空） ---------- */
async function drawOne(){
  if(roundLocked){alert("本轮已结束，请等待下一轮。");return;}
  if(pool.length===0){alert("本轮红包已抽完！"); roundLocked=true; await syncSave(); return;}

  // 云端：同一ID每轮一次，总次数≤3
  if(!userDraws[userID]) userDraws[userID]={count:0,locked:false,0:false,1:false,2:false};
  if(userDraws[userID].locked || userDraws[userID].count>=3){ alert("您已达到本活动最多3次的抽奖上限。"); return; }
  if(userDraws[userID][roundIndex]){ alert("您本轮已经抽过红包！"); return; }

  const v=pool.shift();                        // 发放金额
  userDraws[userID][roundIndex]=true;          // 标记本轮已抽
  userDraws[userID].count+=1;                  // 总次数 +1
  ...
  // 抽空则自动结束本轮
  if(pool.length===0){ roundLocked=true; }
