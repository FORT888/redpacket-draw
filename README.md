...
<title>随机红包抽奖（三轮顺序解锁 + 烟花特效）</title>
<style>
...
<div class="wrap">
  ...
  <div class="card" style="margin-top:20px">
    ...
    <button class="btn secondary" id="reset">重置（管理员）</button>
    <button class="btn warn disabled" id="next">开始下一轮（管理员确认）</button>
  ...
  <div class="card" style="margin-top:20px;text-align:left">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      <thead>
        <tr><th>#</th><th>时间</th><th>轮次</th><th style="text-align:right">金额</th></tr>
      </thead>
...
<script>
const PRESETS=[
  {name:"第 1 轮",total:500,count:3,min:8.8},  // ✅ 改为3个名额
...
function drawOne(){
  if(roundLocked){alert("当前轮未开放，等待管理员开始下一轮");return;}
  if(pool.length===0){alert("本轮已抽完，等待管理员开始下一轮");return;}
  if(userDrawMap[roundIndex]){alert("您本轮已经抽过一次啦，下轮开放后再来～");return;}
  ...
  animateAmount(v);
  save();render();
  if(pool.length===0){roundLocked=true;save();render();alert(`${PRESETS[roundIndex].name} 已抽完，请管理员点击“开始下一轮”。`);els.next.classList.remove("disabled");}
}

function startNext(){
  if(pool.length>0){alert("当前轮尚未抽完");return;}
  if(roundIndex>=PRESETS.length-1){alert("已是最后一轮");return;}
  roundIndex+=1;generateRound(roundIndex);
  alert(`${PRESETS[roundIndex].name} 已开始！`);els.next.classList.add("disabled");
}

function resetAll(){
  if(!confirm("确定要重置吗？这会清空所有记录并重新开始。"))return;
...
function render(){
  ...
  els.quotaInfo.textContent=userDrawMap[roundIndex]?"您本轮已抽过":"您本轮尚未抽";
